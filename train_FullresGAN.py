...
parser.add_argument('-g_lr', '--gen_learning_rate', type = float, default = 2e-3, help = 'learning rate for the optimizer of generator')
...
LOG_DIR=os.path.join(".", "LOG_FullresGAN", CURR_TIMESTAMP)
if CONTINUE_TRAINING: # continue training from last training experiment
    list_of_files = glob.glob(os.path.join(".", "LOG_FullresGAN", "*"))
...
with tf.Session(config=config) as sess:
    ...
    real_out_D, _ = model.discriminator_model(inp=im_pl, training=training_pl, reuse=True, minibatch=MINIBATCH_DISCRIMINATION) # get discriminator output on real images
