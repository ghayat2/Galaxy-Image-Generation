...
class StackedSRM:
        ...
        with tf.variable_scope("StackedSRM"): # define variable scope
            ...
            for i in range(self.nb_stacks):
                with tf.name_scope("stack"):
                    conv = layers.conv_layer(inter, out_channels=48, filter_size=(4, 4), strides=(2, 2), padding=(1, 1), pad_values=0, use_bias=False)
                    relu = layers.relu_layer(conv)
                    res_module = layers.residual_module_srm(relu, training, out_channels=48, nb_blocks=6, pad_values=0, use_bias=False)
                    ...
                    conv1 = layers.conv_layer(up_sample1, out_channels=96, filter_size=(3, 3), strides=(1, 1), padding=(1, 1), pad_values=0, use_bias=False)
    ...
    def train_op(self, losses, learning_rate, beta1, beta2):
