...
if CONTINUE_TRAINING: # continue training from last training experiment
    list_of_files = glob.glob(os.path.join(".", "LOG_MCGAN", "*"))
...
with tf.Session(config=config) as sess:
    ...
    feats_pl = tf.placeholder(dtype=tf.float32, shape=[BATCH_SIZE, 1]) # placeholder for feats fed to generator and discriminator
...
#    sys.exit(0)
    ...
    if CONTINUE_TRAINING:  # restore variables from saved model
        ...
        for filename in glob.glob(os.path.join(CHECKPOINTS_PATH,
                                               "model*")):  # remove all previously saved checkpoints (for limited disk space)
            os.remove(filename)
        saver.save(sess, os.path.join(CHECKPOINTS_PATH, "model"),
                   global_step=global_step_val)  # save the restored model (i,e keep the last checkpoint in this new run)
    ...
    with trange(int(NUM_EPOCHS * (NUM_SAMPLES // BATCH_SIZE))) as t:
        for i in t:  # for each step
            ...
            if (i + 1) % LOG_ITER_FREQ == 0:
                ...
                _, summary = sess.run([discr_train_op, discr_loss_summary],
                                      feed_dict_train)  # train D and get loss_summary as well
                global_step_val = sess.run(discr_global_step)
                writer.add_summary(summary, global_step_val)

                _, summary = sess.run([gen_train_op, gen_loss_summary],
                                      feed_dict_train)  # train G and get loss_summary as well
                global_step_val = sess.run(gen_global_step)
                writer.add_summary(summary, global_step_val)
            ...
            if (i + 1) % SAMPLE_ITER_FREQ == 0:
                feed_dict_test = {training_pl: False, noise_pl: noise_test, feats_pl:feats_test}  # feed dict for testing
