...
class FullresGAN:
    ...
    def discriminator_model(self, inp, training, reuse=False, minibatch=False): # construct the graph of the discriminator
        ...
        with tf.variable_scope("discriminator", reuse=reuse): # define variable scope to easily retrieve vars of the discriminator
            ...
            conv2 = layers.conv_block_fullres(conv1, training, out_channels=8, filter_size=(3, 3), strides=(2, 2), padding="same", use_bias=False, alpha=0.3) # shape=(batch_size, 8, 250, 250)
            conv3 = layers.conv_block_fullres(conv2, training, out_channels=16, filter_size=(3, 3), strides=(2, 2), padding="same", use_bias=False, alpha=0.3) # shape=(batch_size, 16, 125, 125)
            conv4 = layers.conv_block_fullres(conv3, training, out_channels=32, filter_size=(3, 3), strides=(2, 2), padding="same", use_bias=False, alpha=0.3) # shape=(batch_size, 32, 63, 63)
            conv5 = layers.conv_block_fullres(conv4, training, out_channels=64, filter_size=(3, 3), strides=(2, 2), padding="same", use_bias=False, alpha=0.3) # shape=(batch_size, 64, 32, 32)
            conv6 = layers.conv_block_fullres(conv5, training, out_channels=128, filter_size=(3, 3), strides=(2, 2), padding="same", use_bias=False, alpha=0.3) # shape=(batch_size, 128, 16, 16)
            conv7 = layers.conv_block_fullres(conv6, training, out_channels=256, filter_size=(3, 3), strides=(2, 2), padding="same", use_bias=False, alpha=0.3) # shape=(batch_size, 256, 8, 8)
            conv8 = layers.conv_block_fullres(conv7, training, out_channels=512, filter_size=(3, 3), strides=(2, 2), padding="same", use_bias=False, alpha=0.3) # shape=(batch_size, 512, 4, 4)
